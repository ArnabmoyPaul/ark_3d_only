"""
engine_3d.py  —  run10
Multi-task training with loss normalisation (Fix C).

Fix C — Loss scale normalisation (CRITICAL)
  At random init:
    OrganMNIST3D  CE  loss = ln(11) = 2.40
    Binary BCE         loss = ln(2)  = 0.69
    Ratio = 3.5× — Organ dominated all shared encoder gradients

  Fix: divide each task loss by ln(n_classes) before backward
    normalised_loss = raw_loss / ln(n_classes)
    All tasks now contribute ~1.0 scale gradients to the shared encoder

Also removed:
  - Bogus ema_update_teacher(model, model, 1.0) call
  - BN freeze hack (no longer needed — using GroupNorm)
"""
...
from trainer_3d   import evaluate, test_classification
...
def _criterion(tt):
    return torch.nn.CrossEntropyLoss() if _is_multiclass(tt) else \
...
def train_one_epoch_multitask(model, loaders, criterions,
                               loss_weights, optimizer, device):
    """
    Round-robin multi-task training.
    Each task loss is normalised by ln(n_classes) so all contribute
    equal-scale gradients to the shared encoder.
    """
    model.train()
    total_loss = 0.0
    total_n    = 0
    ...
    for _ in range(n_batches):
        for ds_idx in range(n_datasets):
            ...
            # Raw task loss
            raw_loss = criterions[ds_idx](pred, targets)

            # Normalise by ln(n_classes) — equalises gradient scales
            # Binary (1 output): ln(2)=0.693  → weight ~1.44
            # 3-class:           ln(3)=1.099  → weight ~0.91
            # 11-class:          ln(11)=2.398 → weight ~0.42
            norm_loss = raw_loss * loss_weights[ds_idx]
            ...
            total_loss += raw_loss.item() * s1.size(0)
            total_n    += s1.size(0)
...
def engine_3d(args,
    ...
    # Windows: no multiprocessing workers (pagefile DLL crash)
    import platform
    ...
    criterions       = [_criterion(datasets_config[ds]['task_type'])
                        for ds in dataset_list]
    num_classes_list = [len(datasets_config[ds]['diseases'])
                        for ds in dataset_list]

    # Loss normalisation weights: 1 / ln(n_classes)
    # For binary head (n_classes=1 in config), actual n_classes=2 for BCE
    loss_weights = []
    for ds in dataset_list:
        n = len(datasets_config[ds]['diseases'])
        effective_n = max(n, 2)   # binary heads have 1 output but 2 classes
        w = 1.0 / math.log(effective_n)
        loss_weights.append(w)

    print(f"\nLoss normalisation weights:")
    for ds, w, n in zip(dataset_list, loss_weights, num_classes_list):
        print(f"  {ds:<22s}  n_cls={n}  weight={w:.4f}")
    ...
    print(f"\nEpochs: {args.pretrain_epochs}  |  LR: {args.lr}  |  "
          f"Batch: {args.batch_size}  |  WD: {getattr(args,'weight_decay',0.05)}")
    print(f"{'─'*65}")
    ...
    for epoch in range(start_epoch, args.pretrain_epochs):
        ...
        train_loss = train_one_epoch_multitask(
            model, loaders_train, criterions,
            loss_weights, optimizer, device)
        ...
        if (epoch + 1) % args.test_epoch == 0 or \
           epoch + 1 == args.pretrain_epochs:
            ...
            for i, ds_name in enumerate(dataset_list):
                ...
                y, p = test_classification(
                    model, i, loaders_test[i], device, mc)

                if mc:
                    y_idx = y.cpu().numpy().argmax(axis=1)
                    p_idx = p.cpu().numpy().argmax(axis=1)
                    aucs  = metric_AUROC(y, p, n)
                    score = float(np.mean(aucs)) if aucs else \
                            accuracy_score(y_idx, p_idx)
                else:
                    aucs  = metric_AUROC(y, p, n)
                ...
                status = ('✓' if gap >= -0.03 else
                          '~' if gap >= -0.10 else '✗')
                print(f"  {ds_name:<22s} {score:>8.4f}  "
                      f"{sota_v:>6.3f}  {gap:>+7.4f}  {status}")
                auc_row.append(round(score, 4))

                if score > best_auc[ds_name]:
                    best_auc[ds_name] = score
                    save_checkpoint(ckpt,
                        filename=save_stem + f'_best_{ds_name}')
    ...
    for ds in dataset_list:
        s  = best_auc[ds]
        st = SOTA.get(ds, 0.0)
        flag = '✓' if s - st >= -0.03 else '~' if s - st >= -0.10 else '✗'
        print(f"{ds:<22s} {s:>8.4f}  {st:>6.3f}  {s-st:>+7.4f}  {flag}")
    print(f"{'='*65}\n")
